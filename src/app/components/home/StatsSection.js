...
export default function StatsSection() {
  ...
  return (
    <section className="relative w-full py-20 px-6 md:px-12 lg:px-24 overflow-hidden bg-[#E6DFE7]">
      <div className="absolute -top-16 -left-16 w-40 h-40 bg-[#F3E8FF] rounded-full opacity-30 animate-bounce"></div>
      <div className="absolute -bottom-20 -right-20 w-56 h-56 bg-[#EAFBF3] rounded-full opacity-30 animate-pulse"></div>
      <div className="max-w-7xl mx-auto relative z-10 text-center">
        <h2 className="text-3xl md:text-5xl font-bold text-secondary-text mb-6">
        ...
        <div className="w-[90%] md:w-[80%] lg:w-[70%] xl:w-[60%] m-auto rounded-3xl mt-12 flex flex-col md:flex-row gap-6 p-8 justify-between items-center transition-all duration-300">
          {[
            ...
            <div
              key={item.label}
              className="flex flex-col items-center flex-1 px-4 md:border-r-2 border-[#8B5E83] last:border-none"
            >
              ...
              <p className="text-[#5E4B56] text-center mt-2 font-medium">{item.label}</p>
            </div>
          ))}
        </div>
      </div>
